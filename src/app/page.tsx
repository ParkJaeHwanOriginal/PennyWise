...
export default function Dashboard() {
  ...
  const fetchMainData = async () => {
    ...
    if (ledgerData) {
      ...
      if (lastData) {
        // 수익과 예수금은 매일 새로 확인해야 하므로 0으로 초기화해서 이월
  ...
  // 날짜/달 변경 감지
  useEffect(() => {
    if (isMounted && !showSplash) fetchMainData();
  }, [currentDate]);

  useEffect(() => {
    if (isMounted && !showSplash) fetchBudgetData();
  }, [budgetMonthStr]);

  // Touch handlers
  const handleTouchStart = (e: React.TouchEvent) => {
    if (isEditMode || showSplash || !isAuthenticated) return; 
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (isEditMode || !isAtTopOnStart.current || showSplash || !isAuthenticated) return;
    const diff = e.touches[0].pageY - startY.current;
    if (window.scrollY <= 0 && diff > 0) setPullDistance(Math.min(diff * 0.4, 100));
  };
  const handleTouchEnd = () => {
    if (isEditMode || !isAtTopOnStart.current || showSplash || !isAuthenticated) return;
    if (pullDistance > 60) { setIsRefreshing(true); setPullDistance(60); fetchMainData(); fetchBudgetData(); } else { setPullDistance(0); }
  ...
  const handlePinClick = (num: string) => {
    if (pinCode.length < 4) {
      ...
      setPinCode(newPin);
      if (newPin.length === 4) {
        if (newPin === "0519") setTimeout(() => setIsAuthenticated(true), 200);
        else {
          setPinError(true);
  ...
  const handlePinDelete = () => setPinCode(prev => prev.slice(0, -1));
  ...
  const closeLedgerModal = () => {
    ...
    setEditingTx(null); setTxAmount(''); setTxItemName(''); setTxType('-');
  };
  const handleLedgerTransaction = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const amountNum = Number(txAmount);
    let finalAmount = selectedLedgerAccount === 'card_bill' ? amountNum : (txType === '+' ? amountNum : -amountNum);
  ...
  const deleteTransaction = async (tx: any) => {
    ...
    let revertAmount = selectedLedgerAccount === 'card_bill' ? -tx.amount : (tx.type === '+' ? -tx.amount : tx.amount);
    const newBalance = (ledger[selectedLedgerAccount] || 0) + revertAmount;
    await updateLedgerDirect(selectedLedgerAccount, newBalance);
    await supabase.from('ledger_transactions').delete().eq('id', tx.id);
    fetchMainData();
  };
  const startEditTx = (tx: any) => { setEditingTx(tx); setTxType(tx.type); setTxAmount(tx.amount.toString()); setTxItemName(tx.item_name); };
  ...
  const handleTempSave = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setDetails({
      assets: itemData.type === '자산' ? [...filterOut(details.assets), itemData].sort((a,b)=>b.amount-a.amount) : filterOut(details.assets),
      liabilities: itemData.type === '부채' ? [...filterOut(details.liabilities), itemData].sort((a,b)=>b.amount-a.amount) : filterOut(details.liabilities)
    });
      ...
      {!isAuthenticated ? (
        ...
        <div onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} className="min-h-screen bg-slate-50 text-slate-900 font-sans overflow-x-hidden pb-24 relative">
          ...
          <header className="fixed top-0 left-0 w-full bg-white/95 backdrop-blur-md border-b z-[150] flex flex-col shadow-sm">
            ...
            <div className="overflow-hidden bg-slate-50/90 border-t border-slate-100" style={{ maxHeight: isChartVisible || activeTab !== 'dashboard' ? 0 : '40px', opacity: isChartVisible || activeTab !== 'dashboard' ? 0 : 1, transition: 'max-height 0.3s' }}>
            ...
            {activeTab === 'budget' && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                <div className="flex justify-center items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-6 relative">
                  ...
                  <div className="absolute right-4 flex items-center justify-center p-2 bg-slate-50 rounded-full">
                    <input type="month" value={budgetMonthStr} onChange={(e) => setBudgetMonthStr(e.target.value)} className="opacity-0 absolute inset-0 w-full h-full cursor-pointer" />
                    <Calendar size={18} className="text-blue-600" />
                  </div>
                </div>

                <section className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl shadow-sm p-4 relative border border-blue-100 flex justify-between items-center">
                  <div>
                    <span className="text-xs font-black text-blue-800 tracking-tighter block">이번 달 예산 잔여금</span>
                    <span className="text-[9px] text-blue-500 font-bold">가계부 '최종 여윳돈'에 합산됨</span>
                  </div>
                ...
                <section className="space-y-4">
                  <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-5">
                    <label className="text-xs font-black text-slate-400 block mb-2 uppercase tracking-tighter">월급 (총 수입)</label>
                    <div className="flex items-center gap-2 border-b pb-2">
                      <input type="number" value={budget.income || ''} onChange={(e) => handleUpdateBudget('income', Number(e.target.value))} placeholder="0" className="flex-1 bg-transparent text-2xl font-black outline-none" />
                      <span className="font-black text-slate-400">원</span>
                    </div>
                  </div>
                  <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-5">
                    <label className="text-xs font-black text-slate-400 block mb-2 uppercase tracking-tighter">목표 저금액</label>
                    <div className="flex items-center gap-2 border-b pb-2">
                ...
                <section>
                  <h3 className="font-black text-lg italic mb-3 px-1">고정비용</h3>
                  <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                    <div className="p-4 bg-slate-50 border-b">
                      <form onSubmit={handleAddFixedExpense} className="flex gap-2">
                        <input name="name" required placeholder="항목" className="flex-1 p-3 rounded-xl border text-sm font-bold min-w-0" />
                        <input name="amount" required type="number" placeholder="금액" className="w-24 p-3 rounded-xl border text-sm font-bold min-w-0" />
                        <button type="submit" className="bg-slate-900 text-white p-3 rounded-xl"><Plus size={18} /></button>
                      </form>
                    </div>
                    {fixedExpenses.map(exp => (
                      <Reorder.Group axis="y" values={fixedExpenses} onReorder={handleReorderFixedExpenses} key="fixed-reorder">
                        <Reorder.Item value={exp} key={exp.id} className="flex justify-between items-center p-4 border-b last:border-0 bg-white">
                          <div className="flex items-center gap-2"><GripVertical size={16} className="text-slate-300" /><span className="font-bold text-sm">{exp.name}</span></div>
                          <div className="flex items-center gap-3"><span className="font-black text-red-600 text-sm">-{exp.amount.toLocaleString()}원</span><button onClick={() => handleDeleteFixedExpense(exp.id)} className="text-slate-400 border-l pl-2"><Trash2 size={14} /></button></div>
                        </Reorder.Item>
                      </Reorder.Group>
                    ))}
                  </div>
              ...
              <div className="fixed inset-0 z-[300] flex items-end justify-center">
                ...
                <motion.form onSubmit={handleTempSave} initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="relative w-full max-h-[85%] bg-white rounded-t-[2.5rem] shadow-2xl px-6 pt-4 pb-6 flex flex-col max-w-md">
                  ...
                  <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-2">{isEditMode || selectedItem?.isNew ? <Unlock size={18} className="text-blue-600"/> : <Lock size={18} className="text-slate-400"/>}<h3 className="text-xl font-black">{selectedItem?.isNew ? '항목 추가' : (isEditMode ? '항목 수정' : '상세 정보')}</h3></div>
                    <div className="flex items-center gap-2">
                      {isEditMode && !selectedItem?.isNew && <button type="button" onClick={() => handleTempDelete(selectedItem.id)} className="p-2 text-red-400"><Trash2 size={20}/></button>}
                  ...
                  <div className="space-y-4 overflow-y-auto pr-1 no-scrollbar pb-2">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 ml-1">기관</label><select name="bank" disabled={!isEditMode && !selectedItem?.isNew} defaultValue={selectedItem?.bank || '기타'} className="w-full bg-slate-50 rounded-xl p-3 text-sm font-black outline-none">{BANKS.map(b => <option key={b} value={b}>{b}</option>)}</select></div>
                      <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 ml-1">종류</label><select name="type" disabled={!isEditMode && !selectedItem?.isNew} defaultValue={selectedItem?.type} className="w-full bg-slate-50 rounded-xl p-3 text-sm font-black outline-none"><option value="자산">자산</option><option value="부채">부채</option></select></div>
                    </div>
                    <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 ml-1">항목명</label><input name="name" required disabled={!isEditMode && !selectedItem?.isNew} type="text" defaultValue={selectedItem?.name} className="w-full bg-slate-50 rounded-xl p-3 text-sm font-black outline-none" /></div>
                    <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 ml-1">금액</label>
                      {selectedItem?.name?.toUpperCase() === 'ISA' ? <div className="w-full bg-slate-100 rounded-xl p-3 text-sm font-black text-slate-400">가계부 탭에서 관리됩니다</div> : <input name="amount" required min="0" disabled={!isEditMode && !selectedItem?.isNew} type="number" defaultValue={selectedItem?.amount} className="w-full bg-slate-50 rounded-xl p-3 text-sm font-black outline-none" />}
                    </div>
                    <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 ml-1">메모</label><textarea name="memo" disabled={!isEditMode && !selectedItem?.isNew} defaultValue={selectedItem?.memo} className="w-full bg-slate-50 rounded-xl p-3 text-sm font-black h-20 outline-none resize-none" /></div>
                    {(isEditMode || selectedItem?.isNew) && <button type="submit" className="w-full bg-green-600 text-white py-4 rounded-xl font-black shadow-lg">Apply Changes</button>}
                  </div>
                </motion.form>
              </div>
            )}
          ...
          <AnimatePresence>
            {isLedgerModalOpen && (
              <div className="fixed inset-0 z-[300] flex items-end justify-center">
                ...
                <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="relative w-full h-[85vh] bg-white rounded-t-[2.5rem] shadow-2xl px-6 pt-4 pb-8 flex flex-col max-w-md overflow-hidden">
                  <div className="w-12 h-1 bg-slate-200 rounded-full mx-auto mb-6 shrink-0" />
                  <div className="flex justify-between items-center mb-4 shrink-0">
                    <h3 className="text-xl font-black">{getAccountName(selectedLedgerAccount)}</h3>
                    <button type="button" onClick={closeLedgerModal} className="p-2 bg-slate-100 rounded-full"><X size={20}/></button>
                  </div>
                  <div className="flex-1 overflow-y-auto no-scrollbar space-y-6 flex flex-col pb-10">
                    <div className="bg-slate-50 p-4 rounded-2xl border shrink-0">
                      <label className="text-[10px] font-black text-slate-400 block mb-2 tracking-tighter">현재 잔액 (기본금 수정)</label>
                      <div className="flex items-center gap-2"><input type="number" value={ledger[selectedLedgerAccount] || ''} onChange={(e) => updateLedgerDirect(selectedLedgerAccount, Number(e.target.value))} className="flex-1 bg-white p-3 rounded-xl font-black border text-lg min-w-0" placeholder="0" /><span>원</span></div>
                    </div>

                    {selectedLedgerAccount === 'isa' && (
                      <div className="grid grid-cols-2 gap-3 shrink-0 mt-4">
                        <div className="bg-slate-50 p-4 rounded-2xl border flex flex-col"><label className="text-[10px] font-black text-slate-400 block mb-2">예수금</label><input type="number" value={ledger.isa_deposit || ''} onChange={(e) => updateLedgerDirect('isa_deposit', Number(e.target.value))} className="bg-transparent font-black text-lg outline-none" /></div>
                        <div className="bg-blue-50 p-4 rounded-2xl border flex flex-col"><label className="text-[10px] font-black text-blue-400 block mb-2">총 수익</label><input type="number" value={ledger.isa_profit || ''} onChange={(e) => updateLedgerDirect('isa_profit', Number(e.target.value))} className="bg-transparent font-black text-lg outline-none text-blue-600" /></div>
                      </div>
                    )}

                    {selectedLedgerAccount !== 'isa' && (
                      <form onSubmit={handleLedgerTransaction} className="space-y-4 shrink-0 bg-white border p-4 rounded-2xl shadow-sm">
                        {editingTx && <p className="text-[10px] font-black text-blue-600 border-b pb-2 mb-2">내역 수정 모드</p>}
                        <div className="flex gap-2">
                          {selectedLedgerAccount === 'card_bill' ? <div className="flex bg-red-50 p-1 rounded-xl w-[120px] shrink-0 items-center justify-center border border-red-100 text-red-600 font-black text-xs">💳 사용</div> : <div className="flex bg-slate-100 p-1 rounded-xl w-[120px] shrink-0"><button type="button" onClick={() => setTxType('-')} className={`flex-1 rounded-lg text-xs font-black ${txType === '-' ? 'bg-white text-red-600 shadow' : 'text-slate-400'}`}>- 지출</button><button type="button" onClick={() => setTxType('+')} className={`flex-1 rounded-lg text-xs font-black ${txType === '+' ? 'bg-white text-blue-600 shadow' : 'text-slate-400'}`}>+ 수입</button></div>}
                          <input value={txAmount} onChange={(e) => setTxAmount(e.target.value)} required type="number" placeholder="금액" className="flex-1 bg-slate-50 p-3 rounded-xl font-black outline-none border min-w-0" />
                        </div>
                        <input value={txItemName} onChange={(e) => setTxItemName(e.target.value)} required type="text" placeholder="사용 내역" className="w-full bg-slate-50 p-3 rounded-xl font-black outline-none border min-w-0" />
                        <button type="submit" className={`w-full ${editingTx ? 'bg-blue-600' : 'bg-slate-900'} text-white py-4 rounded-xl font-black`}>{editingTx ? '수정 내용 저장' : '기록 추가하기'}</button>
                      </form>
                    )}

                    {selectedLedgerAccount !== 'isa' && (
                      <div className="flex-1">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase mb-3 border-b pb-2">오늘의 내역</h4>
                        <Reorder.Group axis="y" values={dailyTransactions.filter(tx => tx.account === selectedLedgerAccount)} onReorder={handleReorder} className="space-y-2">
                          {dailyTransactions.filter(tx => tx.account === selectedLedgerAccount).map(tx => (
                            <Reorder.Item key={tx.id} value={tx} className="flex justify-between items-center p-3 rounded-xl border bg-white shadow-sm gap-2">
                              <div className="flex items-center gap-2 flex-1 min-w-0"><GripVertical size={16} className="text-slate-300" /><span className="font-bold text-sm truncate">{tx.item_name}</span></div>
                              <div className="flex items-center gap-2 shrink-0"><span className={`font-black text-sm ${tx.type === '+' || selectedLedgerAccount === 'card_bill' ? 'text-blue-600' : 'text-red-600'}`}>{selectedLedgerAccount === 'card_bill' ? '+' : (tx.type === '+' ? '+' : '-')}{tx.amount.toLocaleString()}</span><div className="flex items-center gap-1 border-l pl-2"><button type="button" onClick={() => startEditTx(tx)} className="p-1.5 text-slate-400"><Pencil size={14} /></button><button type="button" onClick={() => deleteTransaction(tx)} className="p-1.5 text-slate-400"><Trash2 size={14} /></button></div></div>
                            </Reorder.Item>
                          ))}
                        </Reorder.Group>
                      </div>
          ...
          <nav className="fixed bottom-0 w-full bg-white/90 backdrop-blur-lg border-t pb-[env(safe-area-inset-bottom)] z-[200]">
            <div className="flex justify-around items-center h-[70px] px-6 max-w-md mx-auto">
              <button onClick={() => setActiveTab('dashboard')} className={`flex flex-col items-center gap-1.5 transition-colors ${activeTab === 'dashboard' ? 'text-blue-600' : 'text-slate-400'}`}><PieChartIcon size={22} strokeWidth={2.5} /><span className="text-[9px] font-bold">Dashboard</span></button>
              <button onClick={() => setActiveTab('ledger')} className={`flex flex-col items-center gap-1.5 transition-colors ${activeTab === 'ledger' ? 'text-blue-600' : 'text-slate-400'}`}><BookOpen size={22} strokeWidth={2.5} /><span className="text-[9px] font-bold">Ledger</span></button>
              <button onClick={() => setActiveTab('budget')} className={`flex flex-col items-center gap-1.5 transition-colors ${activeTab === 'budget' ? 'text-blue-600' : 'text-slate-400'}`}><Calculator size={22} strokeWidth={2.5} /><span className="text-[9px] font-bold">Budget</span></button>
            </div>
          </nav>
        </div>
      )}
