...
const getDbDateString = (date: Date) => date.toISOString().split('T')[0];
...
export default function Dashboard() {
  ...
  const [budgetMonthStr, setBudgetMonthStr] = useState(() => getMonthString(new Date('2026-03-24')));
  ...
  const fetchMainData = async () => {
    ...
    if (ledgerData) {
      ...
      const { data: prevData } = await supabase.from('daily_ledger').select('*').eq('date', getDbDateString(prevDate)).single();
      setLedger(prevData ? { ...prevData, date: dbDate } : { date: dbDate, isa: 0, isa_profit: 0, isa_deposit: 0, card_bill: 0, kb_bank: 0, nonghyup: 0, kakao: 0, naver: 0, cash: 0, kb_pointree: 0 });
    }
